...
use crate::{
    ...
    testing::utils::{
        executes::sudo_update_pause,
        helpers::MIN_FUNDS,
        setup::{MAX_BET, MIN_BET},
    },
...
fn test_update_bank_limit() {
    ...
    sudo_update_bank_limit(&mut app, contract_addr.clone(), NATIVE_DENOM, new_limit).unwrap();

    let config = query_config(&app, contract_addr).unwrap();
    let bank_limit = config.denom_limits.get(NATIVE_DENOM).unwrap().bank;

    assert_eq!(bank_limit, new_limit);
}
...
fn test_no_flip_same_block() {
    ...
    .unwrap_err();
    assert_eq!(err, ContractError::NoFlipsToDoThisBlock);
...
fn test_missing_funds() {
    ...
    assert_eq!(
        err,
        ContractError::ContractMissingFunds(NATIVE_DENOM.to_string())
    );
}
