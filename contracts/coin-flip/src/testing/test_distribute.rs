...
fn test_distribute() {
    ...
    let total_fee_amount_to_pay = query_fees(&app, contract_addr.clone(), NATIVE_DENOM).unwrap();
    ...
    let dry_dist = query_dry_distribution(&app, contract_addr.clone(), NATIVE_DENOM).unwrap();
    assert_eq!(
        dry_dist,
    ...
    query_dry_distribution(&app, contract_addr.clone(), NATIVE_DENOM).unwrap();
    let res = sudo_distribute(&mut app, contract_addr.clone(), NATIVE_DENOM).unwrap();
    let res_data = get_dist_result(res);

    let total_fee_amount_left = query_fees(&app, contract_addr.clone(), NATIVE_DENOM).unwrap();
...
fn test_distribute_without_collection() {
    ...
    update_storage(&mut app, contract_addr.as_bytes(), &mut |storage| {
        let fees = FEES.load(storage, NATIVE_DENOM.to_string()).unwrap();
        FEES.save(
            storage,
            NATIVE_DENOM.to_string(),
            &fees.checked_add(Uint128::new(1)).unwrap(),
        )
        .unwrap();
    });
    ...
    let total_fee_amount = query_fees(&app, contract_addr.clone(), NATIVE_DENOM).unwrap();
    assert_eq!(
        total_fee_amount,
        total_fee_amount_to_pay
            .checked_sub(res_data.team_paid)
...
fn test_failing_distribute() {
    ...
    query_dry_distribution(&app, contract_addr.clone(), NATIVE_DENOM).unwrap_err();
    let err = sudo_distribute(&mut app, contract_addr.clone(), NATIVE_DENOM).unwrap_err();
    ...
    query_dry_distribution(&app, contract_addr.clone(), NATIVE_DENOM).unwrap_err();
    let err = sudo_distribute(&mut app, contract_addr, NATIVE_DENOM).unwrap_err();
