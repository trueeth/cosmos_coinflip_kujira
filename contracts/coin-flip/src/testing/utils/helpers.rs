...
use cosmwasm_std::{coin, coins, Addr, Decimal, Empty, Uint128};
use cosmwasm_storage::PrefixedStorage;
use cw_multi_test::{AppResponse, Executor};

use crate::types::PickTypes;

use super::{
    executes::{execute_start_flip, unwrap_execute},
    setup::{BaseApp, MIN_BET, NATIVE_DENOM, USDC_DENOM},
};
    ...
    app.init_modules(|router, _, storage| {
        router
            ...
            .init_balance(
                storage,
                &addr,
                vec![coin(amount, NATIVE_DENOM), coin(amount, USDC_DENOM)],
            )
            .unwrap();
    });
}
...
pub fn mint_nfts(app: &mut BaseApp, nft_contract_addr: Addr, sender: Addr, to_mint: u64) {
    add_balances(app, to_mint);
